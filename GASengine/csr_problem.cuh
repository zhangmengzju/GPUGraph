...
//               Program::expandOverEdges() == EXPAND_IN_EDGES))
          ...
          int word_size = (graph_slices[0]->nodes + 30) / 31; // for compression extension, must be multiple of 31
...
//               Program::expandOverEdges() == EXPAND_IN_EDGES))
          ...
          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_edgefrontier, 0, word_size * sizeof (unsigned int)),
                                        "Memset d_bitmap_edgefrontier failed", __FILE__, __LINE__))
            return retval;

          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_out, 0, word_size * sizeof (unsigned int)),
                                        "Memset d_bitmap_out failed", __FILE__, __LINE__))
            return retval;

          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_prefix, 0, word_size * sizeof (unsigned int)),
                                        "Memset d_bitmap_prefix failed", __FILE__, __LINE__))
            return retval;

          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_assigned, 0, word_size * sizeof (unsigned int)),
                                        "Memset d_bitmap_assigned failed", __FILE__, __LINE__))
            return retval;

          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_in, 0, word_size * sizeof (unsigned int)),
                                        "Memset d_bitmap_out failed", __FILE__, __LINE__))
            return retval;

          if (retval = util::B40CPerror(cudaMemset(graph_slices[0]->d_bitmap_visited, 0, word_size * sizeof (unsigned int)),
