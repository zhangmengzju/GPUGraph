...
__global__ void kernel_scatter_mgpu(int frontier_selector, int move_count, int num_active, int* d_edge_frontier_size, typename Program::SizeT* offsets, typename Program::VertexId* active_vertices,
    typename Program::VertexId* edge_count_scan, typename Program::VertexId* indices, const int* mp_global, typename Program::VertexId* edge_frontier, typename Program::VertexType vertex_list,
...
//          d_edge_frontier_size[frontier_selector] += move_count;
  ...
  // interval sizes.
  int4 range = mgpu::CTALoadBalance<NT, VT>(move_count, edge_count_scan, num_active, block, tid, mp_global, indices_shared, true);
...
//            printf("blockIdx.x=%d, threadIdx.x=%d, vertex_id=%d, edge_id=%d, neighbor=%d\n", blockIdx.x, threadIdx.x, vertex_id, edge_id, neighbor);
      expand_edge_functor(true, 0, vertex_id, neighbor, read_edge_id, vertex_list, edge_list, data[i], local_misc_values[i]);
    }
  }

  mgpu::DeviceRegToGlobal<NT, VT>(move_count, data, tid, edge_frontier + range.x);

  mgpu::DeviceRegToGlobal<NT, VT>(move_count, local_misc_values, tid, misc_values + range.x);
}

template<typename Program>
__global__ void kernel_apply(const int rank_id, long long iteration, int frontier_size, typename Program::VertexId* frontier, typename Program::GatherType *gather_values,
    typename Program::VertexType vertex_list, typename Program::EdgeType edge_list, char* changed)
{
  ...
  for (int i = tidx; i < frontier_size; i += gridDim.x * blockDim.x)
  {
    ...
    typename Program::GatherType gather_value = gather_values[i]; //gather_values[v];
//		printf("In apply kernel: vertex_id=%d, gathervalue=%d\n", v, gather_value);
    apply_functor(rank_id, v, iteration, gather_value, vertex_list, edge_list, changed[v]);
...
__global__ void kernel_gather_mgpu(typename Program::SizeT num_active, const typename Program::VertexId *active_vertices, const typename Program::SizeT numBlocks, typename Program::SizeT move_count,
    const typename Program::SizeT *edge_count_scan, const typename Program::VertexId *mp_global, const typename Program::SizeT *offsets, const typename Program::VertexId *indices,
    typename Program::VertexType vertex_list, typename Program::EdgeType edge_list, typename Program::VertexId *d_edgeCSC_indices, typename Program::VertexId *pred_vertices,
...
#pragma unroll
  for (int i = 0; i < VT; ++i)
  {
    ...
    if (index < move_count)
    {
      ...
      {
        if (d_edgeCSC_indices)
          gather_edge_functor(vertex_id, d_edgeCSC_indices[edge_id], neighbor, vertex_list, edge_list, result[i]);
        else
          gather_edge_functor(vertex_id, edge_id, neighbor, vertex_list, edge_list, result[i]);
      }
//                  else {
...
//                  }
  ...
  mgpu::DeviceRegToGlobal<NT, VT>(move_count, result, tid, gather_results + range.x);

  mgpu::DeviceRegToGlobal<NT, VT>(move_count, predecessors, tid, pred_vertices + range.x);
}
...
//			typename Program::VertexId vertex_id = predecessors[i];
//
//			if (offsets[vertex_id + 1] - offsets[vertex_id] > 0) {
//				if (d_edgeCSC_indices)
//					gather_edge_functor(vertex_id, d_edgeCSC_indices[edge_id],
//							neighbor, vertex_list, edge_list, result[i]);
//				else
...
template<typename Program>
struct PredicatedEdgeCountIterator: public std::iterator<std::input_iterator_tag, typename Program::SizeT>
{
  typename Program::SizeT *m_offsets;
  typename Program::SizeT *m_active;
  typename Program::SizeT *m_predicates;

  __host__ __device__ PredicatedEdgeCountIterator(typename Program::SizeT *offsets, typename Program::SizeT *active, typename Program::SizeT * predicates) :
      m_offsets(offsets), m_active(active), m_predicates(predicates)
  {
  }
  ;

  __device__ typename Program::SizeT operator[](typename Program::SizeT i) const
  {
    typename Program::SizeT active = m_active[i];
    return m_predicates[i] ? m_offsets[active + 1] - m_offsets[active] : 0;
  }

  __device__ PredicatedEdgeCountIterator operator +(typename Program::SizeT i) const
  {
...
template<typename Program>
struct ThrustReduceWrapper: std::binary_function<typename Program::GatherType, typename Program::GatherType, typename Program::GatherType>
{
  __device__ typename Program::GatherType operator()(const typename Program::GatherType &left, const typename Program::GatherType &right)
  {
...
__global__ void reset_gather_result(int iteration, int num_elements, int* frontier, typename Program::VertexType vertex_list, typename Program::EdgeType edge_list,
    typename Program::GatherType* gather_tmp, typename KernelPolicy::VisitedMask* d_visited_mask) // Per-CTA clock timing statistics (used when KernelPolicy::INSTRUMENT)
{
...
template<typename KernelPolicy, typename Program>
__global__ void reset_gather_result(int iteration, int nodes, typename Program::VertexType vertex_list, typename Program::EdgeType edge_list, typename Program::GatherType* gather_tmp,
...
__global__
void kernel_copy_if(InputIt in, int N, PredicateIt pred, int *d_map, OutputIt output)
{
