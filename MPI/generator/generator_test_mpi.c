...
int main(int argc, char* argv[])
{
  ...
  if (rank == 0) fprintf(stderr,
		  "np=%d, Graph size is %" PRId64 " vertices and %" PRId64 " edges as %d partitions\n",
		  size, INT64_C(1) << log_numverts, edges_per_vert << log_numverts, ppr*size);
...
if (rank == size - 1)
{
    ...
    for (int i = 0; i < size; i++)
    {
		for(int j=0;j<ppr;j++)
		{
			...
			printf("sizes for rank %d: %d\n", i, writelength[i*ppr+j]);
		}
    }
