...
template<typename VertexId, typename Value, typename SizeT>
  void
  CPUBFS(int test_iteration, const CsrGraph<VertexId, Value, SizeT> &csr_graph,
...
void
printUsageAndExit(char *algo_name)
{
...
void
MPI_init(int argc, char** argv, int &device_id, int& rank, int& numprocs)
{
  int devCount;
  char idstr[256];
  char idstr2[256];
...
//		device_id = rank % devCount;
    ...
    else
    {
      ...
      else
      {
        ...
        sprintf(idstr, "%-11s %5d %4d %s", processor_name, rank, devCount,
            devProp.name);
      }
    }
...
//	printf("inside\n");
  ...
  if (graph_random == false && graph_type == 1)
  {
    ...
    if (rank_id == 0)
    {
      ...
      long long num_vert_per_part_1d = (csr_graph.nodes + num_part_1d - 1)
          / num_part_1d;
      ...
      {
        long long buffer[2] =
        { part_count[i], num_vert_per_part_1d };
        MPI_Isend(buffer, sizeof(long long) * 2, MPI_CHAR, i, 0, MPI_COMM_WORLD,
            &request[0]);
        MPI_Wait(&request[0], &status[0]);
        MPI_Isend(coos[i], sizeof(EdgeTupleType) * part_count[i], MPI_CHAR, i,
      ...
      csr_graph.FromCoo<true>(coos[rank_id], num_vert_per_part_1d,
          part_count[rank_id], !directed);
    ...
    else
    {
      ...
      long long buffer[2];
      MPI_Irecv(buffer, sizeof(long long) * 2, MPI_CHAR, 0, 0, MPI_COMM_WORLD,
          &request[0]);
      MPI_Wait(&request[0], &status[0]);
      ...
      MPI_Irecv(coos, part_size * sizeof(EdgeTupleType), MPI_CHAR, 0, 0,
  ...
  else if (graph_random == false && graph_type == 2)
  {
    ...
    if (rank_id == 0)
    {
      ...
      vector<EdgeTupleType*> coos;
      vector<long long> part_count;
      csv_graph_reader<g_with_value, VertexId, Value, SizeT> reader(graph_file,
          true);
  ...
  else if (graph_random == false)
  {
    ...
    int i;
    int length[np];
    int rc = MPI_File_open(MPI_COMM_WORLD, "Graphsizes", MPI_MODE_RDONLY,
        MPI_INFO_NULL, &sizefile);
    if (rc)
    {
    ...
    {
      if (edges[2 * i] >= numvert1d || edges[2 * i + 1] >= numvert1d)
        printf("\nNot in range %d %d  should be in %d\n", edges[2 * i],
            edges[2 * i + 1], numvert1d);
  ...
  if (strcmp(source_file_name, ""))
  {
    if (strcmp(source_file_name, "RANDOM") == 0)
    {
      ...
      while (count < num_srcs)
      {
        ...
        if (csr_graph.row_offsets[tmp_src + 1] - csr_graph.row_offsets[tmp_src]
            > 0)
        {
          srcs[count++] = tmp_src;
        }
...
//  }
  ...
  int num_gpus = 1;
  typedef GASengine::CsrProblem<bfs, VertexId, SizeT, Value, g_mark_predecessor,
      g_with_value> CsrProblem;
  ...
  if (csr_problem.FromHostProblem(g_stream_from_host, csr_graph.nodes,
      ...
      csr_graph.edge_values, csr_graph.row_indices, csr_graph.column_offsets,
      num_gpus, directed, device_id, rank_id))
    exit(1);
