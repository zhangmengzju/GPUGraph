...
void CPUBFS(int test_iteration, const CsrGraph<VertexId, Value, SizeT> &csr_graph, VertexId *source_path, VertexId src)
{
  ...
  std::cout << "CPU time took: " << (EndTime - startTime) * 1000 << " ms" << std::endl;
  search_depth++;
}
...
void printUsageAndExit(char *algo_name)
{
  ...
  std::cout << "     -parameters (-p) set the options.  Options include the following:\n";
  Config::printOptions();
...
void MPI_init(int argc, char** argv, int &device_id, int& rank, int& numprocs)
{
  int devCount;
  char idstr[256];
  char idstr2[256];
...
//	printf("inside\n");
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (strncmp(argv[i], "-graph", 100) == 0 || strncmp(argv[i], "-g", 100) == 0)
    { //input graph
      i++;
...
//		if (rank_id == 0)
    ...
    if (rank_id == 0) csr_graph.DisplayGraph();
    delete[] coo;
    delete[] edges;
//		if (rank_id == 0)
  ...
  if (strcmp(source_file_name, ""))
  {
    ...
    else
    {
      ...
      for (num_srcs = 0; num_srcs < max_src_num; num_srcs++)
      {
        ...
        {
          if (origin == 1) srcs[num_srcs]--; //0-based index
        }
        else
          break;
      }
...
//  }
  ...
  if (csr_problem.FromHostProblem(g_stream_from_host, csr_graph.nodes, csr_graph.edges, csr_graph.column_indices, csr_graph.row_offsets, csr_graph.edge_values, csr_graph.row_indices,
      csr_graph.column_offsets, num_gpus, directed, device_id, rank_id)) exit(1);
...
//  for (int i = 0; i < num_srcs; i++)
  for (int i = 0; i < 1; i++)
  {
    ...
    retval = vertex_centric.EnactIterativeSearch(csr_problem, csr_graph.row_offsets, directed, tmp_num_srcs, tmpsrcs, iter_num, threshold, np, device_id, rank_id);
